...
class LiquidCmsTest < Test::Unit::TestCase
  ...
  def test_empty_form_builder
    ...
    template = '{% formfor text activity:edit %}{% endformfor %}'
  ...
  def test_form_builder_element
    ...
    template = '{% formfor liquid_demo_model activity:edit %}<br/>{{ form_login }}{% endformfor %}'
  ...
  def test_form_builder_class
    ...
    template = '{% formfor user activity:edit class:tester %}{% endformfor %}'
  ...
  def test_form_error_default
    ...
    template = '{% formfor user activity:edit class:tester %}{{ form_login }}{% endformfor %}'
    assert_template_result(expected, template, 'user' => user_drop)  
  ...
  def test_form_scope
    user_drop = get_drop @@user_default_values
    expected = '<form method="POST" class="tester" action="' + (object_url user_drop) + '"><input type="hidden" name="_method" value="PUT"/></form>'
    template = '{% formfor user activity:edit class:tester %}{% endformfor %}{{form_login}}'
    assert_template_result(expected, template, 'user' => user_drop)  
  end  
  ...
  def test_form_builder_class
    ...
    template = '{% formfor user activity:edit %}{% endformfor %}'
    assert_template_result(expected, template, 'user' => user_drop)
  end  
